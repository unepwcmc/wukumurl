...
class ShortUrlsControllerTest < ActionController::TestCase
  ...
  test "PUT update should not update the URLs short_name if user is not the owner" do
    ...
    put( :update, id: short_url.id, short_url: {short_name: 'zzz'} )

    short_url.reload
    assert_equal short_url.short_name, "xxx"
  end

  test "PUT update should update the URLs short_name if user is the owner" do
    user = FactoryGirl.create(:user)
    sign_in user
  ...
  test "PUT update should not update the URLs short_name if user is not signed in" do
    short_url = FactoryGirl.create(:short_url, short_name: "xxx")

    id = ShortUrl.last[:id]
    short_url = {:short_name => "zzz"}
    put( :update, {id: id, short_url: short_url} )
    assert_equal ShortUrl.last.short_name, "xxx"
  end
end
