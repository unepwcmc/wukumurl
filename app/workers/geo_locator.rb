...
class GeoLocator
  ...
  def update_orgs_by_short_url visit
    ...
    existing_short_url = CartoDB::Connection.query("
      SELECT COUNT(*)
      FROM #{CARTODB_CONFIG['tables']['organizations_by_short_url']}
    ...
    if existing_short_url[:rows].first[:count] > 0
      short_url_query = "
        UPDATE #{CARTODB_CONFIG['tables']['organizations_by_short_url']}
        SET visits=visits+1
        WHERE short_url_id = #{short_url.id}
        AND org_id=#{organization.id}
      "
      ...
      short_url_query = "
        INSERT INTO #{CARTODB_CONFIG['tables']['organizations_by_short_url']}
  ...
  def update_orgs_by_user visit
    ...
    if short_url.user_id
      existing_user = CartoDB::Connection.query("
        SELECT COUNT(*)
        FROM #{CARTODB_CONFIG['tables']['organizations_by_user']}
      ...
      if existing_user[:rows].first[:count] > 0
        user_query = "
          UPDATE #{CARTODB_CONFIG['tables']['organizations_by_user']}
          SET visits=visits+1
          WHERE user_id = #{short_url.user_id}
          AND org_id=#{organization.id}
        "
        ...
        user_query = "
          INSERT INTO #{CARTODB_CONFIG['tables']['organizations_by_user']}
  ...
  def update_orgs visit
    ...
    existing_org = CartoDB::Connection.query("
      SELECT COUNT(*)
      FROM #{CARTODB_CONFIG['tables']['visits_by_organization']}
      WHERE org_id=#{organization.id}
    ")

    if existing_org[:rows].first[:count] > 0
      org_query = "
        UPDATE #{CARTODB_CONFIG['tables']['visits_by_organization']}
        SET visits = visits + 1
        WHERE org_id=#{organization.id}
      "
    else
      org_query = "
        INSERT INTO #{CARTODB_CONFIG['tables']['visits_by_organization']}
